...
final readonly class ReferenceAnalyzer
{
    ...
    public function analyze(string $path): array
    {
        ...
        $docBlocks = $docs->getDoc() ?? [];

        // Extract template parameter names to exclude from class references
        $templateNames = DocProcessor::extractTemplateNames($docBlocks);

        // Merge and deduplicate references from all sources
        $allReferences = array_merge(
            $imports->getImports() ?? [],
            $names->getNames() ?? [],
            DocProcessor::process($docBlocks),
        );

        // Filter out template parameter names (not actual classes)
        $filtered = array_filter(
            $allReferences,
            fn (string $class): bool => !in_array($class, $templateNames, true),
        );
