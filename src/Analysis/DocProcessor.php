...
final class DocProcessor
{
    ...
    /**
     * Extract template parameter names from PHPDoc blocks.
     *
     * Scans @template tags to identify generic type parameter names that should not
     * be treated as class references during analysis.
     *
     * @param  array<DocBlock> $docs Collection of parsed PHPDoc blocks to analyze
     * @return array<string>   List of template parameter names (e.g., ['T', 'TKey', 'TValue'])
     */
    public static function extractTemplateNames(array $docs): array
    {
        $templates = [];
        ...
        foreach ($docs as $doc) {
            foreach ($doc->getTags() as $tag) {
                if ($tag->getName() === 'template') {
                    /** @template tags have format: "@template T" or "@template T of SomeClass" */
                    $description = (string) $tag;

                    // Extract just the template name (first word after @template)
                    if (preg_match('/@template\s+([A-Za-z_][\w]*)/', $description, $matches)) {
                        $templates[] = $matches[1];
                    }
                }
            }
        ...
        if (method_exists($tag, 'getParameters')) {
            ...
            if (is_array($parameters)) {
                foreach ($parameters as $param) {
                    if (is_object($param) && method_exists($param, 'getType')) {
                        $type = $param->getType();
