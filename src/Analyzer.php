...
final readonly class Analyzer
{
    ...
    public function analyze(): array
    {
        ...
        throw_if(!$pathResolver instanceof PathResolverInterface || !$reporter instanceof ReporterInterface, RuntimeException::class, 'Required resolvers and reporter must be configured');
        ...
        $callback = function (SplFileInfo $file) use ($reporter): AnalysisResult {
            try {
                ...
                // Include stack trace for debug output (-vvv)
                if ($this->config->verbosity === Verbosity::Debug) {
                    $errorMessage .= "\n\nStack trace:\n".$throwable->getTraceAsString();
                }

                $result = AnalysisResult::error($file, $errorMessage);
            }
