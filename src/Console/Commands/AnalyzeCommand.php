...
final class AnalyzeCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        /** @var array<string> $ignoreOption */
        $ignoreOption = $this->option('ignore');

        if ($ignoreOption !== []) {
            Config::set('analyzer.ignore', $ignoreOption);
        }

        /** @var array<string> $excludeOption */
        $excludeOption = $this->option('exclude');

        if ($excludeOption !== []) {
            Config::set('analyzer.exclude', $excludeOption);
        }
        ...
        $pathResolver = resolve($pathResolverClass);
        ...
        $fileResolver = resolve($fileResolverClass);
        ...
        if ($this->option('lang')) {
            ...
            $analysisResolverClass = Config::get('analyzer.translation.resolver', TranslationAnalysisResolver::class);
            $analysisResolver = resolve($analysisResolverClass);
        } elseif ($this->option('route')) {
            /** @var class-string $analysisResolverClass */
            $analysisResolverClass = Config::get('analyzer.route.resolver', RouteAnalysisResolver::class);
            $analysisResolver = resolve($analysisResolverClass);
        } else {
            /** @var class-string $analysisResolverClass */
        ...
        $reporter = $agentMode ? resolve(AgentReporter::class) : resolve($reporterClass);
        ...
        if ($this->option('parallel')) {
            ...
            $processor = resolve($processorClass);
