...
Route::prefix('api/v1')->name('api.v1.')->group(function (): void {
    ...
    Route::get('/posts', fn () => 'v1 posts')->name('posts.index');
    Route::post('/auth/login', fn () => 'v1 login')->name('auth.login');
});

Route::prefix('api/v2')->name('api.v2.')->group(function (): void {
    Route::get('/users', fn () => 'v2 users')->name('users.show');
    Route::get('/posts/{id}', fn () => 'v2 post')->name('posts.show');
});
